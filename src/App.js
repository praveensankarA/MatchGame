...
class App extends Component {
  state = {
    ...
    score: 0,
    isResultPage: false,
    currentItem: imagesList[0],
    categoryOfItem: tabsList[0].tabId,
  }

  componentDidMount() {
    this.intervalId = setInterval(this.secondChange, 1000)
  }
  ...
  changeCategory = tab => {
    const {categoryOfItem} = this.state
    this.setState({categoryOfItem: tab})
  }

  suffleMatchpicture = () => {
    const newItem = imagesList[Math.floor(Math.random() * imagesList.length)]
  ...
  playAgainFunction = () => {
    this.setState({isResultPage: false, score: 0, timer: 60})
    clearInterval(this.intervalId)
    this.intervalId = setInterval(this.secondChange, 1000)
    this.setState({currentItem: imagesList[0]})
  }

  imageCheck = url => {
    const {score, currentItem} = this.state

    if (currentItem.thumbnailUrl === url) {
      this.setState({score: score + 1})
      this.suffleMatchpicture()
    } else {
      this.result()
    }
  }
  ...
  render() {
    const {timer, score, currentItem, isResultPage, categoryOfItem} = this.state

    const filteredList = imagesList.filter(e => e.category === categoryOfItem)
    console.log(categoryOfItem)
    if (score === 30) {
      this.result()
    }
    return (
        ...
        <div className="main-bg-container">
          {!isResultPage && (
            <>
              ...
              <ul className="img-items-container">
                {filteredList.map(each => (
                  <li key={each.thumbnailUrl}>
                    <button
                      type="button"
                      onClick={() => this.imageCheck(each.thumbnailUrl)}
                      className="list-ele-btn"
                    >
                      <img
                        src={each.thumbnailUrl}
                        alt="thumbnail"
                        className="list-image-ele"
                      />
                    </button>
                  </li>
                ))}
          ...
          {isResultPage && (
            <div className="result-page-main-container">
              <img
                ...
                className="trophy"
              />
              <p className="result-page-score-text">YOUR SCORE</p>
              <h1>{score}</h1>
              <button
                type="button"
